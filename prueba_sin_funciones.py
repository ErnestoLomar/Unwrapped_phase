...
for r in range(rows):
    for c in range(cols):
        ...
        unwrapped_phase[r, c] = d_phi2[r, c] + np.arctan2(delta_r, delta_c)

        # Perform 2π jumps corrections
        while (unwrapped_phase[r, c] - d_phi2[r, c]) > threshold:
            unwrapped_phase[r, c] -= 2 * np.pi

        while (unwrapped_phase[r, c] - d_phi2[r, c]) < -threshold:
            unwrapped_phase[r, c] += 2 * np.pi

fase_desenvuelta = unwrapped_phase
...
# Compute the 2D unwrapped phase using Fourier transform
unwrapped_phase_takeda = np.real(np.fft.ifft2(-laplacian))

# Adjust the phase to be in the range [-pi, pi]
unwrapped_phase_takeda = np.mod(unwrapped_phase_takeda + np.pi, 2 * np.pi) - np.pi

unwrapped_phase_map = unwrapped_phase_takeda
